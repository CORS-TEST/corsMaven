...
if (!nexacro.Grid) {
	...
	_pGridCell.on_apply_custom_setfocus = function (evt_name, self_flag) {
		...
		if (self_flag || nexacro._enableaccessibility || (!this._no_dom_focus && !(nexacro.OS == "iOS" && grid.autoenter == "select" && this._hasEditor()))) {
					...
					if ((datarow == grid._selectinfo.curdsrow) && (cell._cellidx == grid._selectinfo.curcell)) {
						if (grid._showEditing) {
							...
							if (upelem) {
								...
								if (check) {
									cell.on_fire_onclick(button, altKey, ctrlKey, shiftKey, screenX, screenY, canvasX, canvasY, clientX, clientY, obj, from_refer_comp, cell._subComp ? "control" : "", true);
								}
							}
						}
						else {
	...
	_pGridEdit.on_apply_custom_setfocus = function (evt_name, callback) {
		...
		var ret = nexacro.EditCtrl.prototype.on_apply_custom_setfocus.call(this, evt_name);


		if (callback && !this._displaymode && !this.autoselect) {
			this.setCaretPos(0);
		}
		return ret;
	};
	...
	_pGridMaskEdit.on_apply_custom_setfocus = function (evt_name, callback) {
		var onceTime = this._grid._onceTime_focus;
		if (nexacro.Browser == "MobileSafari" && (nexacro._isHybrid && nexacro._isHybrid())) {
			onceTime = true;
		}

		if (onceTime && !callback) {
			nexacro.OnceCallbackTimer.callonce(this, function () {
				this.on_apply_custom_setfocus(evt_name, true);
			}, 0);
			return;
		...
		var ret = nexacro.MaskEditCtrl.prototype.on_apply_custom_setfocus.call(this, evt_name);
	...
	_pGridCellCheckbox._find_pseudo_obj = function (styleProp, pseudo, returnType) {
		...
		if (this._cellobj && pseudo != "disabled") {
			if (this._grid._isFakeCell(datarow) || grid._is_updaterow) {
				pseudo = "normal";
			}
		}
	...
	_pGridTree.on_find_CurrentStyle_align = function (pseudo) {
		var align = nexacro.Component.prototype.on_find_CurrentStyle_align.call(this, pseudo);


		if (this._grid && (align && !align.valign)) {
			var temp_align = this._grid.on_find_CurrentStyle_align(pseudo);
	...
	_pGrid.on_fire_onlbuttonup = function (button, altKey, ctrlKey, shiftKey, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem, user_fire, need_recalcXY) {
		if (this._focus_proc) {
			if (!this._showEditing) {
				...
				if (this._currentCellEditor._cellobj._rowidx == this._focus_proc.parent._rowidx) {
	...
	_pGrid.on_fire_user_onkeydown = function (keyCode, altKey, ctrlKey, shiftKey, obj, refer_comp) {
			...
			if (edittype == "textarea" && ((!editacceptsenter && (altKey || ctrlKey || shiftKey)) || (editacceptsenter && !altKey && !ctrlKey && !shiftKey))) {
				...
				if ((refer_comp instanceof nexacro.ComboEditCtrl || refer_comp instanceof nexacro.CalendarEditCtrl)
					...
					if (this._setdataobj && this._setdataobj.succ == false) {
						...
						if (this.autoenter == "select") {
							if (refer_comp && refer_comp._cellobj) {
								var cellobj = refer_comp._cellobj;
		...
		if (keyCode == nexacro.Event.KEY_UP) {
			...
			if (nexacro.Browser == "Edge" && this._showEditing) {
	...
	_pGrid._isDownActionKeyMouse = function () {
		...
		if (_win && (_win._cur_ldown_elem || this._keydown_elem)) {
			var elem = _win._cur_ldown_elem || this._keydown_elem, comp = _win.findComponent(elem, 0, 0)[0], isgrid = false;
				...
				if (this._beforeEditRowIdx != currow || this._beforeEditCellIdx != curcell) {
					if (cellobj._hasEditor()) {
						...
						if (editor) {
							...
							if (!this._onceTime_focus && editor.setCaretPos) {
