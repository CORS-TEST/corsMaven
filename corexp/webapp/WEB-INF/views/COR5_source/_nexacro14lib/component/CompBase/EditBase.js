...
if (!nexacro.EditBase) {
	...
	if (nexacro.Browser == "IE" && nexacro.BrowserVersion <= 10 && nexacro._getRealBrowserVersion() <= 10) {
			...
			if (nexacro.Browser == "IE" && nexacro.BrowserVersion == 10) {
				_pEditBase._on_default_input_keyinput = function (elem) {
					...
					if ((nexacro.SystemLang == "ja" || nexacro.SystemLang == "ja-JP") && 
						mouse_stat == "mousedown" && !(nexacro.Browser == "Edge" && this._is_composition())) {
		...
		_pEditBase._on_default_input_keyinput = function (elem) {
			var mouse_stat = this._stat_mouse.getCurrentStatus();
			if ((mouse_stat == "mousedown" && nexacro.Browser != "Edge") || ((nexacro.SystemLang == "ja" || nexacro.SystemLang == "ja-JP") && 
				mouse_stat == "mousedown" && !(nexacro.Browser == "Edge" && this._is_composition()))) {
				elem.setElementValue(this._value);
				return false;
			}
	...
	if (nexacro.Browser == "Edge" || (nexacro.Browser == "IE" && (nexacro.BrowserVersion == 11 || (nexacro.BrowserType == "IE10" && nexacro._getRealBrowserVersion() == 11)))) {
		_pEditBase._on_default_input_compositionend = function (data) {
			...
			if (nexacro.Browser == "Edge" && data.length > 1) {
				var compositiondata = "";
				var len = data.length;
				for (var i = 0; i < len; i++) {
					if (!this._is_hangul(data[i])) {
		...
		_pEditBase._on_default_input_compositionend = function (data) {
			...
			if (this._accept_keyinput_event) {
				if (this._is_hangul(this._stat_composition.getPreviousData())) {
					input_val = this._stat_composition.getPreviousData();
				}
				this._fire_text_event(input_val);
				return true;
			}
	...
	_pEditBase._is_selected = function (elem) {
		if (!elem) {
			...
			if ((nexacro.Browser == "Edge" || (nexacro.Browser == "IE" && nexacro.BrowserVersion >= 11)) && !this._pasteAction) {
				var elem_value = elem.getElementValue();
	...
	_pEditNormal._on_default_input_keydown = function (elem, keyCode, altkey, ctrlkey, shiftkey) {
		...
		if (keyCode == nexacro.KeyCode_ImeInput) {
			...
			if (nexacro.Browser == "IE" && nexacro.BrowserVersion == 8) {
				...
				if (nexacro.Browser == "Edge" && this._stat_composition.getCurrentStatus() == nexacro.EditBase.Status.CompositionEnd) {
					var data = this._stat_composition.getData();
					if (this._is_hangul(data)) {
						this._on_input_compositionend(data);
						this._fire_text_event(data);
	...
	_pEditNormal._on_default_input_keypress = function (elem, keyCode, charCode, altKey, ctrlKey, shiftKey) {
		...
		if (k == nexacro.Event.KEY_BACKSPACE || (k == nexacro.Event.KEY_DELETE && k == this._keycode) || (nexacro.Browser == "MobileSafari" && k == 127)) {
			...
			if ((nexacro.Browser == "Edge" || (nexacro.Browser == "IE" && nexacro.BrowserVersion >= 11)) && k == nexacro.Event.KEY_BACKSPACE) {
				this._set_input_begin_pos(this._begin_pos);
			}
	...
	_pEditNormal._keyup_process_enter = function (elem) {
		...
		if (nexacro.Browser == "Edge" || nexacro.OS == "iOS") {
			var elem_val = elem.getElementValue();
	...
	if (nexacro.OS != "iOS" && nexacro.OS != "Mac OS") {
	...
	if (nexacro.Browser == "Edge" || nexacro.Browser == "IE") {
		...
		_pEditMaskString._on_default_input_keyinput = function (elem) {
			var mouse_stat = this._stat_mouse.getCurrentStatus();
			if ((nexacro.SystemLang == "ja" || nexacro.SystemLang == "ja-JP") && (mouse_stat == "mousedown" && !(nexacro.Browser == "Edge" && this._is_composition()))) {
				elem.setElementValue(this._value);
				return false;
			}
			...
			if (!this.isLimit(strText)) {
				if (this._dispComma) {
					...
					if (nOldLength != nNewLength) {
						...
						if (nexacro.Browser == "Chrome" && this._is_composition()) {
							this._accept_keyinput_event = false;
						}
					}
				}
