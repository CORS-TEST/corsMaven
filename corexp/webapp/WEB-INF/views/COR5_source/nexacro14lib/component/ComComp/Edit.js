...
if (!nexacro.Edit) {
	...
	_pEdit.on_apply_custom_setfocus = function (evt_name) {
		...
		if (api && input_elem) {
			...
			if (evt_name != "lbuttondown" && this._old_selection[0] != this._old_selection[1]) {
				...
				if (!((nexacro.Browser == "Edge" || nexacro.Browser == "IE" || nexacro.Browser == "Runtime") && evt_name == "lbuttondown")) {
					if (this._setcaret) {
						this._setcaret = false;
					}
			...
			if ((nexacro.Browser == "Chrome" || nexacro.Browser == "Safari") && evt_name == "lbuttondown" && !this._is_subcontrol) {
	...
	_pEdit._on_killfocus = function (obj, fromObj) {
		...
		if (api && elem) {
			...
			if ((nexacro.Browser == "IE" || nexacro.Browser == "Edge" || nexacro.Browser == "Gecko") && 
	...
	_pEdit._cancelSelect = function () {
		...
		else if (nexacro.Browser == "Runtime" || nexacro.Browser == "Chrome") {
			if (api._is_selected()) {
				this.setCaretPos(api._begin_pos);
			}
		}
