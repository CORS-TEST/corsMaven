...
<div class="body">
	...
	<form name="tx_editor_form" id="tx_editor_form" action="http://posttestserver.com/post.php" method="post" accept-charset="utf-8">
		<!-- 에디터 컨테이너 시작 -->
		<div id="tx_trex_container" class="tx-editor-container tx-editor-fullscreen">
			<!-- 사이드바 -->
...
<script type="text/javascript">
	var config = {
		txHost: 'http://<%=request.getServerName()%>:<%=request.getServerPort()%>', /* 런타임 시 리소스들을 로딩할 때 필요한 부분으로, 경로가 변경되면 이 부분 수정이 필요. ex) http://xxx.xxx.com */
	...
	//kspark - Nexacro 소스 추가
	if (! window.NEXACROHTML) {
		window.NEXACROHTML = {};
	}
	
	window.NEXACROHTML.FireUserNotify = function(userdata,type) {
			if (window.NEXACROWEBBROWSER) {
				if (type == "GET"){
					window.NEXACROWEBBROWSER.on_fire_onusernotify(window.NEXACROWEBBROWSER, "▦"+userdata);	
				} else if (type = "SET"){
					window.NEXACROWEBBROWSER.on_fire_onusernotify(window.NEXACROWEBBROWSER, "▦"+bInit);
				}
		  	} else {
		  		if (type == "GET"){
		  			window.document.title = "▩"+userdata;	
		  		} else if (type = "SET"){
		  			window.document.title = "▩"+bInit;	
		  		}
			}			
		}
	...
	function getContent(str){
		try {
		    window.NEXACROHTML.FireUserNotify(str,"GET");
		} catch(e) {
			alert(e + "error");
		}
    }
		...
		try {
			var interval = setInterval(fn = function() {
		    	if(bInit) {
		    		...
		    		window.NEXACROHTML.FireUserNotify("","SET");
			    }
		  	}, 100);
		} catch(e) {
			alert(e + "error");
	...
	function setEditor(str){
		Editor.getCanvas().setContent(str);
	}
</script>
